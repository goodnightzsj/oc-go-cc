...
- 当前：14/14；AWS表头缺译中英文DOM、#5全量及#6部署增量验证均已完成；真实账户条件仍保留在#8。
- 真源：TODO.csv；主线程负责后端、页面、验证和整合，所有旧代理写入权限失效。
- 下一步：本软件子任务无剩余实施；0b28ab2已部署、#7UI/UX报告完成，#8等待真实账户外部条件。不能用合成验证替代账户权限或未公开API。
...
- 既有合成账单截图显示filter.status；本次恢复重新运行新增DOM回归，英文明确失败。复用已经定义的th.status后英文Status、中文状态通过，未改动账单数据或视觉布局。
- 实际浏览器矩阵加入第三列表头精确断言；发布前全量验证移交#5，不把旧650/1073结果当作增量源码验证。
