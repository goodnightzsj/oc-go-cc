...
The response also carries `model_limits`: the per-model monthly allowance table parsed from the Go docs pages (`internal/quota/limits.go`, `ParseModelLimits`), refreshed daily by `limitsLoop` (24h TTL, lazy first fetch via `ensureModelLimits`, zh page with en fallback). Pricing variants (Peak/Off-Peak, token-range splits) merge into one row per model. Since the upstream usage endpoint never reports per-model numbers, `model_usage` is built from this instance's own SQLite ledger (`monthlyModelUsage` — per-model stored cost within [monthly resets_at − 30d, resets_at), converted to $60-pool equivalents × 60/allowance, the same multiplier the OpenCode ledger applies); the UI shows only models with ledger entries, and the total row is the sum of the rows (pool-equivalent %, comparable to the endpoint's monthly percent, which stays on the account cards).
