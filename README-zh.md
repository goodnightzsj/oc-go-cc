...
**历史请求** 支持按平台、状态、时间、模型、使用场景、请求类型和费用来源筛选，并按任意列排序。Token 列第一行是总数，第二行是提示 / 输出的拆分，不打开弹窗即可看出单次请求的形态。被路由到其他模型时，详情弹窗在服务模型下方标出客户端**请求的模型**，并给出该次请求自身的 Tok/s——与模型明细表同口径，因此一行的数可以与其模型均值对照。命中平台峰时费率的记录带 `高峰 ×N` 角标——角标显示实际计费的倍率，因此在 DeepSeek 计价的平台上读作 `×2`，在 OpenRouter 的 `tencent/hy3` 上读作 `×1.6`——费用按该倍率计算。峰谷适用于各平台自己发布了两档费率的模型：OpenCode Go、CommandCode、ClinePass 计价的 DeepSeek 各行，窗口为三家一致的 UTC 01-04 / 06-10（北京 09:00-12:00 / 14:00-18:00）工作日；以及 OpenRouter 三个 `pricing.overrides` 带时间窗的模型。中国法定节假日按规则排除：OpenCode Go 与 ClinePass 沿用 DeepSeek 的表述，CommandCode 自己写明规则且无此豁免、节假日照计高峰；OpenRouter 的窗口数据里没有节假日概念，同样照计高峰。日历取自国务院公告（每日抓取），并内嵌一份种子到二进制里，因此节假日按低谷计费而不是高峰。日志可通过搜索框按请求模型、服务模型、供应商、场景和错误信息检索。
