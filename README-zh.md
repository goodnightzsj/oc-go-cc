...
各模型每月使用额度每天从 [Go 文档](https://opencode.ai/docs/zh-cn/go) 同步（中文页优先，英文页兜底；Peak/Off-Peak 等定价变体合并为一行）。用量表格只列出本实例确实用过的模型，展示本月原始用量（来自本代理自己的记账，与 opencode 控制台的成本口径一致）、每月配额、占比与窗口总计。
