...
各模型每月使用额度每天从 [Go 文档](https://opencode.ai/docs/zh-cn/go) 同步（中文页优先，英文页兜底；Peak/Off-Peak 等定价变体合并为一行）。用量表格只列出本实例确实用过的模型，展示本月加权用量（来自本代理自己的记账：模型原始消费 × 60/额度 —— $15 额度的模型按 4×、$30 按 2×、$60 按 1× 计入共享的 $60 月池）、每月配额、占比与窗口总计。
