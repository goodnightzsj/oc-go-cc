...
各模型每月使用额度每天从 [Go 文档](https://opencode.ai/docs/zh-cn/go) 同步（中文页优先，英文页兜底；Peak/Off-Peak 等定价变体合并为一行）。用量表格只列出本实例确实用过的模型，展示本月折算为共享 $60 池的等效用量（原始成本 × 60/额度 —— 与 opencode 记账的模型倍率一致）、每月配额、占比与总计行（总计恒等于各模型行之和，可与官方窗口总进度对照）。
