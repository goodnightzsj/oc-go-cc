...
func TestHistoryAssetsKeepKeyboardDialogAndPromptTokens(t *testing.T) {
	...
	for _, marker := range []string{`type="hidden" id="history-start"`, `type="hidden" id="history-end"`, `id="history-date-popover"`, `id="history-summary-requests"`, `id="status-filter"`, `id="streaming-filter"`, `id="cost-source-filter"`, `data-sort="cost_usd"`} {
