...
async function checks() {
  const want = ['opencode-go','commandcode','opencode-zen','aws-bedrock','openrouter'];
  ...
  for (const metric of ['requests','total_tokens','cost_usd']) {
    for (const id of ['overview-provider-distribution','provider-distribution']) {
      ...
      equal(providerOrder(id),[...want,'a-custom','z-custom'],'provider distribution order: '+id+' '+metric);
    }
  }
  ...
  equal(labels('history-provider-breakdown'),[...want,'a-custom','z-custom'],'history platform breakdown');
  ...
  equal([...document.getElementById('fallback-add-model').innerHTML.matchAll(/value="([^"]+)"/g)].map(match=>match[1].split('/')[0]),
    [...want,'a-custom','z-custom'],'add-model picker');
  ...
  equal(TestModule.testModelSelect.children.map(option=>option.value),[...want,'a-custom','z-custom'].map(provider=>'alias-'+provider),'test model picker');
